...
- Training definitions are portable and reference named **content roots** (see `architecture.md` → Content Roots), not absolute paths; each machine maps roots to absolute paths locally. Training content itself is referenced, never copied into app data.
